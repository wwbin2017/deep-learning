...
word_id, label_id, word_num, label_num = data_preprocess.data2id(file_name)
print("Data processing ends ......")
print("字典大小：", word_num, " 词性类别数：", label_num)
...
class POS(object):
    ...
    def __init__(self, config, is_training, embedding_word=None):
        """
        初始化函数
        :param config: 模型的配置参数 
        :param is_training  : 训练还是预测
        :param embedding_word: 训练好的词向量
        """
        self.config = config
        self.config.label_size = label_num + 1
        self.config.wordNum = word_num + 1
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.device("/cpu:0"):
                # 获取词嵌入
                if self.embedding_word is not None:
                    embedding_char = tf.Variable(self.embedding_word,
                                                 trainable=True, name="embedding_char", dtype=tf.float32)
                else:
                    embedding_char = tf.Variable(
                        tf.truncated_normal([self.config.wordNum, self.config.char_dim], stddev=0.1),
                        trainable=True, name="embedding_char", dtype=tf.float32)
    ...
    def save_model(self, save_path):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        # 记录tf模型
        tf_path = os.path.join(save_path, 'pos_vars')
    ...
    def restore(self, path):
        ...
        """
        with self.graph.as_default():
            self.saver.restore(self.sess, path + '/pos_vars')
...
def load_model(model_path="./model_data"):
    ...
    :return: 返回模型 
    """
    pos = POS(ConfigArgs, False, None)
    pos.restore(model_path)
    return pos


def save_model(model_path="./model_data"):
    pos = POS(ConfigArgs, True, None)
    pos.train_model()
    # 保存模型
    pos.save_model(save_path=model_path)


def predict(pos, sentence):
    """
    predict ne
    :param pos: 
    :param sentence: 
    ...
    label_id = pos.predict_char(char_list, model_dict)
    labels = pos.id2label(char_list, label_id, label_rdict)
...
if __name__ == '__main__':
    ...
    pos = load_model()
    sentence = u'我 是 中国 人。结婚 的 和 没 结婚 的'
    labels = predict(pos, sentence)
    print(labels)
